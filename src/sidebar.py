from PySide6.QtWidgets import QFrame, QLabel, QLineEdit, QPushButton, QVBoxLayout, QMessageBox

from websocket_task import WebsocketTask
...
class Sidebar(QFrame):
    def __init__(self, /):
        ...
        # 初始化插件信息
        self.plugin_name = "Live2d Parameter Acquirer"
        self.plugin_developer = "SwarmClone"
    ...
    def on_prepare_button_click(self):
        if self.port is None or not (0 <= self.port <= 65535):
            ...
            msg_box.setText("必须输入一个有效的整数端口号\n请确保输入框内没有非数字字符\n设置的端口号需要在0~65535的范围内")
            msg_box.setStandardButtons(QMessageBox.Ok)
            msg_box.button(QMessageBox.Ok).setText("确认")
            msg_box.exec()
        else:
            print("准备按钮被点击")
            self.wt = WebsocketTask(self.port, self.plugin_name, self.plugin_developer, self.plugin_icon, self.token_path)
    ...
    def on_start_button_click(self):
        print("开始监听按钮被点击")
        if hasattr(self, 'authentication_token') and self.authentication_token:
            print(f"使用 authenticationToken: {self.authentication_token}")
        else:
            # 理论上不会出现这个的……
            QMessageBox.warning(self, "警告", "尚未完成准备，请先点击‘准备’按钮。")